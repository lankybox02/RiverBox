...
setTimeout(function () {
  ...
  function adminLoadPost(data) {
    document.getElementById("pageContent").innerHTML = `<div class="post"><span style="color: var(--secondaryfont);cursor:pointer" onclick="viewUserPage('` + data.author + `')">` + data.author + `</span><span style="margin-top:7px;margin-bottom:7px;display:block;color:var(--postfont)">` + atob(data.content) + `</span>
